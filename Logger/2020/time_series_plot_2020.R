...
for (i in 1:length(list_iButton_corr)){
  if (!any(is.na(list_iButton_corr[[i]][,2]))){
  dataname=names(list_iButton_corr)[i] #save logger ID 
  name=paste(substring(list_iButton_corr[[i]][1,3], 1,10),"time series plot",dataname,".pdf") #set filename
  data=list_iButton_corr[[i]][,2] #retrieve column with temperature data from list
  time_series=ts(data, start=c(list_iButton_corr[[i]][1,2]), frequency = 24*6) #create time series
