#second calibration - controlling for drift
#from the 22.08.2020 01:00:00 to 23.08.2020 15:00:00 
...
# 2020/08/25: adapt script for second calibration/checking for drift
...
setwd("C:/00_Dana/Uni/6. Semester/Bachelorarbeit/logger_data/Calibration_test_20200821-20200823/Logger")
...
iButton_ID=read.table("0800000051790E21_200823.csv", sep = ",", dec = ".", header = F, skip = 4,
...
d_iButton_single = read.csv(text=paste0(head(readLines("0800000051790E21_200823.csv"), -1)), dec = ".", skip = 8, header = F, 
...
#correct time stamp as some iButtons recorded from 01:01:00 instead of 01:00:00
for(i in 1:length(list_iButton_datetime)){
  if(list_iButton_datetime[[i]][1]=="2020-08-22 01:01:00 CEST"){
    list_iButton_datetime[[i]]=list_iButton_datetime[[i]]-60
  }else{}
}
...
start_Labtest=as.POSIXct("2020-08-22 01:00:00")
end_Labtest=as.POSIXct("2020-08-23 15:00:00")

# Apply the time index on the single data table
d_iButton_single_corr <- subset(d_iButton_single, Datetime >= start_Labtest & Datetime <= end_Labtest)

# Apply the time index on each data table in the list "list_iButton"
# So for each data table the same time period is selected
list_iButton_corr = lapply(list_iButton, function(x) {subset(x, x[,1] >= start_Labtest & x[,1] <= end_Labtest)})
...
# Set Working directory (wd)
# setwd(choose.dir()) # Uncomment if necessary, but only works for Windows operating system
setwd("C:/00_Dana/Uni/6. Semester/Bachelorarbeit/logger_data/Calibration_test_20200821-20200823/Converted")
...
# Create vector containing ID #
# We only need the ID # without the first (empty) column, so we set the first column to NULL in colClasses
Tref_header=read.table("TOA5_Temp_Data_1Hz0_2020_08_21_1518.dat", sep = ",", dec = ".", header = F, skip = 1,
                       nrows = 1, as.is = T) 
...
# 1. Read in a single file 
# Adjust file name to the exact file you want to read in!
#d_Tref_single = read.table("TOA5_Temp_Data_1Hz0_2020_08_21_1518.dat", sep = ",", dec = ".", header = F, 
...
d_Tref_corr <- subset(d_Tref, TIMESTAMP >= start_Labtest & TIMESTAMP <= end_Labtest)

# Calculation of mean of the reference thermometers
mean_Tref <- mean(d_Tref_corr[,3]) # = 20.2 for both thermometers
mean_Tref
# Create empty NA vector with length of iButton statistics table
n <- length(d_iButton_stat$Temperature_mean_C) # Define length = length of iButton statistics data dataframe
v_mean_Tref <- rep(NA, n)
# Create a vector containing the mean of the reference thermometers
d_mean_Tref <- ifelse(is.na(v_mean_Tref),mean_Tref, v_mean_Tref)
...
diff_T <- d_mean_Tref[1:length(d_mean_Tref)]- d_iButton_stat$Temperature_mean_C[1:length(d_iButton_stat$Temperature_mean_C)]
...
# Set Working directory (wd)
# setwd(choose.dir()) # Uncomment if necessary, but only works for Windows operating system
setwd("C:/00_Dana/Uni/6. Semester/Bachelorarbeit/logger_data/Calibration_test_20200821-20200823")
...
write.table(d_iButton_stat, file = "iButton_Statistics_readable.csv", row.names = FALSE,
            sep=";", dec=",")
